...
三種模式：
智能中英（Smart）：預設模式，自動判定中文或英文，圖示為「智」。
英文（English）：只處理英文輸入。
中文（Chinese / Zhuyin）：只處理中文注音輸入，圖示為「中」。
...
3.2 安裝指令
以系統管理員執行  SmartZhEnIme-v1.1.6-Setup-x64.exe
遵照畫面說明完成安裝
...
5. 卸載與移除
5.1  建議的乾淨移除流程
先關閉正在使用 SmartZhEnIme 的應用程式。
執行 應用程式的卸載程序
開啟 Windows 語言／鍵盤設定，確認 SmartZhEnIme 已不在鍵盤清單中。
若仍顯示於清單，可手動將該鍵盤移除。
必要時登出再登入 Windows，讓 TSF / icon cache 完整刷新。
若要重新安裝。
...
左上紅框
文字輸入區 + 候選窗
顯示目前輸入內容與候選字清單。候選字仍屬未 commit 狀態，可用 Up/Down 選擇，Enter 提交。
中央視窗
資訊視窗
右鍵點擊輸入法 icon 後選擇「資訊」會開啟。內容包含模式說明、操作說明與隱私聲明。
右下綠框
Windows 鍵盤清單
可看見「繁體中文（台灣）/ CHT智能中英」。若安裝後未生效，通常需要在此處重新加入 SmartZhEnIme。
右下紅框左側
智
代表目前處於 Smart 智能中英模式。
右下紅框右側
ㄆ profile icon
代表 SmartZhEnIme 的 language profile icon。這是 Windows 鍵盤清單／語言列中辨識此輸入法的重要圖示。
